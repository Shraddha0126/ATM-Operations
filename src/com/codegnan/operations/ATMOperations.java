...
public class ATMOperations {
	...
	public static IATMService validateCard(long cardNumber) throws InvalidCardException{
		if(dataBase.containsKey(cardNumber)) {
			...
			ACTIVITY.add("Accessed by: "+cardNumber+"is not compatable");
	...
	public static void resetUserAttempts(IATMService operatorCard) {
		...
		try {
			...
			ACTIVITY.add("Accessed by : "+operatorCard.getUserName()+" : to rest the number of chaarcters");
		}catch(InvalidCardException ive) {
			System.out.println(ive.getMessage());
		}
		}
	public static IATMService validCredentials(long cardNumber,int pinNumber)throws InvalidCardException,InvalidPinException,NotAOperatorException,IncorrectPinLimitReachedException{
		if(dataBase.containsKey(cardNumber)) {
			...
			if(card.getChances()<=0) {
				...
				throw new InvalidPinException("You have entered a wrong pin");
			}else {
				return card;
			}
		}
		public static void validateAmount1(double amount) throws InsufficientMachineBalanceException{
			if(amount>ATM_MACHINE_BALANCE) {
				throw new InsufficientMachineBalanceException("Insufficient cash in the machine");
			}
		}

		public static void validateDepositAmount1(double amount)
		throws InsufficientMachineBalanceException,InvalidAmountException{
			...
			if(amount+ ATM_MACHINE_BALANCE>120000.0d) {
				ACTIVITY.add("unable to deposit cash in ATM Machine....");
				throw new InsufficientMachineBalanceException("You can't deposit cash as linit of the ");
			}
		}
		
		public static void operatorMode1(IATMService card) {
			Scanner scanner=new Scanner(System.in);
			double amount;
			boolean flag=true;
			while(flag) {
				System.out.println("Opearor MODE: Operator Name :"+card.getUserName());
				System.out.println("==========================");
				System.out.println("||		0.Switch Off the machine		||");
				System.out.println("||		1.to check ATM Machine Balance		||");
				System.out.println("||		2.Deposit cash in ATM 		||");
				System.out.println("||		3.Rest user pin attempts		||");
				...
				case 0:
					...
					break;
				case 1:
					ACTIVITY.add("Accessed by:"+card.getUserName()+"Activity Performed: Checking ATM");
					System.out.println("The Balance of ATM Machine is: "+ATM_MACHINE_BALANCE+":Is available");
					break;
				case 2:
					System.out.println();
		...
		public static void main(String[] args) throws NotAOperatorException {
			...
			dataBase.put(222222221L, new AxisDebitCard("Yesudas", 222222221L, 50000.0, 2222));
			dataBase.put(3333333331L, new SBIDebitCard("Saxena", 3333333331L, 55000.0, 3333));
			dataBase.put(4444444441L, new AxisDebitCard("Riya", 4444444441L, 32500.0, 4444));
			dataBase.put(5555555551L, new HDFCDebitCard("Dominee", 5555555551L, 71000.0, 5555));
					...
					while (true) {
						...
						try {
							...
							case 2:
								...
								validateDepositAmount1(depositAmount); // Validate deposit amount
								...
								break;
							case 3:
								System.out.println("Your Account Balance is: " + card.checkAccountBalance()); // Check balance
